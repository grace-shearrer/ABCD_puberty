...
# Load data
```{r}
load("~/Google Drive/ABCD/ABCD_puberty/_data/TOS2020/POST_CLEAN.RData")
```
...
# check
```{r}
check<-c("subjectkey","gender","interview_age" ,"age_years","anthroweightcalc","anthroheightcalc","BMI","BMIsds", "BMIper", "")
ch<-data[check]
head(ch)
# Looks good checked with https://www.cdc.gov/healthyweight/bmi/result.html?&method=english&gender=m&age_y=0&age_m=128&hinches=57.50&twp=65
```
...
```{r}
data %>%
  select(pds_bdyhair_y,pds_ht2_y, pds_skin2_y,pds_m5_y, pds_m4_y) %>%
  summary()
```
```{r}
load("~/Google Drive/ABCD/ABCD_puberty/_data/TOS2020/bak.RData")
```
...
data_m<-data %>%
  subset(gender == "M") %>%
  mutate(PCSum= sum(pds_bdyhair_y,pds_ht2_y, pds_skin2_y,pds_m5_y, pds_m4_y))
summary(data_m$PCSum)
...
data_f<-data %>%
  subset(gender == "F") %>%
  mutate(PCSum= sum(pds_bdyhair_y,pds_ht2_y, pds_skin2_y,pds_f4_2_y, pds_f5_y))
summary(data_f$pds_f5_y)
